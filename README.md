...
1. If your new board was purchased with the BBB already attached then the 5370 software is pre-installed and the board is ready to be plugged in. Otherwise see the section below "Installing your own BBB and software" before proceeding.
...
You could also decide to assign a fixed address to the instrument outside of the range of addresses managed by DHCP. Set this address by using the front panel menu interface as described in the [READ_MORE.md](http://github.com/jks-prv/5370_proc/blob/master/READ_MORE.md) file.
...
Now login to the BBB. From the command line of another computer on your network try "ssh root@ip_address" (e.g. ssh root@192.168.1.2). Just type "return" for the password. Also see the [READ_MORE.md](http://github.com/jks-prv/5370_proc/blob/master/READ_MORE.md) file and this link for other login suggestions: http://elinux.org/Beagleboard:Terminal_Shells
